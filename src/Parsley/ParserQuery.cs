...
public static class ParserQuery
{
    ...
    {
        return (ref ReadOnlySpan<char> input, ref int index, [NotNullWhen(true)] out TValue? succeedingValue, [NotNullWhen(false)] out string? expectation) =>
        {
            expectation = null;
    ...
    {
        return (ref ReadOnlySpan<char> input, ref int index, [NotNullWhen(true)] out U? uValue, [NotNullWhen(false)] out string? expectation) =>
        {
            if (parse(ref input, ref index, out var tValue, out expectation))
                return constructNextParser(tValue)(ref input, ref index, out uValue, out expectation);
